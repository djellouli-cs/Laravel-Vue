    ...
    <table class="w-full border border-collapse text-sm">
      <thead>
        <tr class="bg-gray-100">
          ...
          <th class="border px-2 py-1">Matricule</th>
          <th class="border px-2 py-1">Groupe</th>
        </tr>
      </thead>
      <tbody>
...
// Available options based on filteredNumeros
const availableOrganismes = computed(() => {
  const ids = new Set(filteredNumeros.value.map(num => num.organisme?.id).filter(Boolean))
  return props.organismes.filter(org => ids.has(org.id))
})
const availableDestinations = computed(() => {
  const ids = new Set(filteredNumeros.value.map(num => num.destination?.id).filter(Boolean))
  return props.destinations.filter(dest => ids.has(dest.id))
})
const availableServices = computed(() => {
  const ids = new Set(filteredNumeros.value.map(num => num.service?.id).filter(Boolean))
  return props.services.filter(srv => ids.has(srv.id))
})
const availableTechnologies = computed(() => {
  const ids = new Set(filteredNumeros.value.map(num => num.technologie?.id).filter(Boolean))
  return props.technologies.filter(tech => ids.has(tech.id))
})
const availableGroupes = computed(() => {
  const ids = new Set()
  filteredNumeros.value.forEach(num => {
    if (num.destination?.groupes) {
      num.destination.groupes.forEach(g => ids.add(g.id))
    }
  })
  return props.groupes.filter(groupe => ids.has(groupe.id))
})
...
function exportToWord() {
  ...
  link.href = url
  link.download = 'resultats_filtres.doc'
