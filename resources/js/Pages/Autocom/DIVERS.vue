<template>
  <div class="p-6 min-h-screen bg-gradient-to-br from-blue-50 to-white">
    ...
    <!-- Filters: DIVERS first, then Technologie -->
    ...
    <div v-else>
      <table v-if="filteredAcheminements.length > 0" class="reglette">
        ...
        <tbody>
          <!-- First Row: main NDappel in selected DIVERS -->
          <tr>
          ...
          <!-- Second Row: all other acheminements for same NDappel (any DIVERS) -->
          <tr>
          ...
          <!-- Third Row: other NDappel in the same DIVERS -->
          <tr>
...
export default {
  ...
  computed: {
    availableDIVERSs() {
      ...
      const DIVERSNumbers = new Set();
      results.forEach(a => {
        const match = a.acheminement.match(/DIVERS\s+(\d+)/);
        if (match) DIVERSNumbers.add(parseInt(match[1]));
      });
      return Array.from(DIVERSNumbers).sort((a, b) => a - b);
    }
  },
  methods: {
    onTechnologieChange() {
      this.loadDIVERSData(); // keep selected DIVERS
    },
    loadDIVERSData() {
      this.loading = true;
      setTimeout(() => {
    ...
    getAcheminementsByColumn(column) {
      return this.filteredAcheminements.filter(
        ach => ach.acheminement === `DIVERS ${this.searchNumber} ${column}`
      );
    },

    // new: get all other acheminements for same NDappel, any DIVERS
