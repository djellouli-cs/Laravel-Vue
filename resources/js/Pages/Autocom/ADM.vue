  ...
  <div class="p-6 min-h-screen bg-gradient-to-br from-blue-50 to-white">
    ...
    <div class="flex gap-4 mb-6 flex-wrap">
      ...
      <select v-model="searchNumber" @change="loadADMData" class="px-4 py-2 border rounded-lg text-lg">
        ...
        <option v-for="num in availableADMs" :key="num" :value="num">
          ADM {{ num }}
        </option>
      </select>
    ...
    <div v-else>
      <table v-if="filteredAcheminements.length > 0" class="reglette">
        <thead>
          <tr>
            <th v-for="num in 10" :key="num">{{ num }}</th>
          </tr>
        </thead>
        <tbody>
  ...
  methods: {
    ...
    getAcheminementsByColumn(column) {
      return this.filteredAcheminements.filter(
        ach => ach.acheminement === `ADM ${this.searchNumber} ${column}`
      );
    },
